  ...
  if (rfm->IsConnected()) {
    ...
    if (rfm->GetRadioType() == RFMxx::RadioType::RFM95W) {
      ...
      if (rfm->ToggleInterval) {
        ...
        // muss identisch zur Bit-Maske in WebFrontend BuildRadioCard() sein
        const char* rateLabels[] = { "17.241", "9.579", "8.842", "6.631", "4.800" };
        for (int b = 0; b < 5; b++) {
          if (bitRead(rfm->ToggleMode, b)) {
            if (!first) result += " / ";
            result += rateLabels[b];
            first = false;
          }
...
String HardwarePageBuilder::Build(RFMxx *rfm1, RFMxx *rfm2, RFMxx *rfm3, RFMxx *rfm4, RFMxx *rfm5,
                                   OwnSensors *ownSensors,
                                   SC16IS750 *sc16is750_1, SC16IS750 *sc16is750_2,
                                   DigitalPorts *digitalPorts, Display *display,
                                   DataPort *dataPort1, DataPort *dataPort2, DataPort *dataPort3,
                                   TcpServer *serialBridge, TcpServer *serialBridge2,
                                   TcpServer *softSerialBridge,
  ...
  if (ownSensors->HasBME280()) {
    ...
    cv280t += " ADC: T=" + String(lm280.ADCT);
  ...
  if (ownSensors->HasBMP280()) {
    ...
    cvP280t += " ADC: T=" + String(lmP280.ADCT);
  ...
  if (ownSensors->HasBMP180()) {
    ...
    cv180t += " ADC: T=" + String(lm180.ADCT);
